...
router.post('/crear',verificacion, async (req, res) => {
  ...
  const { nombre, tipo, descripcion, proveedor, precioCompra, capacidad_presentacion, usuario, fecha_caducidad } = req.body;
  try {
    ...
    const producto= new Producto({ nombre, tipo, descripcion, proveedor, precioCompra, capacidad_presentacion, usuario, 
...
router.put('/actualizar/:id', verificacion, async (req, res) => {
  const { id } = req.params;
  const { nombre, tipo, descripcion, proveedor, precioCompra, capacidad_unidad, capacidad_presentacion, usuario, fecha_caducidad} = req.body;
  try {
    const fechaActual = new Date(); 
    const fechaCaducidad = new Date(fecha_caducidad); 
    ...
    const productoActualizado = await Producto.findByIdAndUpdate(id, { nombre, tipo, descripcion, proveedor, precioCompra, capacidad_unidad, 
      capacidad_presentacion, usuario, fecha_caducidad: fechaCaducidad, fecha_actualizacion: fechaActual  }, { new: true });
