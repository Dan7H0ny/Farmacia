...
export const ReporteVenta = (venta, usuario) => {
  const doc = new jsPDF({
    format: 'letter' // Especificar que el formato es carta
  });

  const opciones = {
    titulo: 'HOJA DE LA VENTA',
    nombreArchivo: `Reporte de Venta.pdf`,
    logo: logoBase64,
  ...
  doc.text(`${venta.cliente.stringIdentity.nombre}:`, margin, 57.5);
  doc.text(`FECHA DE REGISTRO:`, margin, 65);
  
  const rightAlignMargin = pageWidth - margin;

  doc.setFont('helvetica', 'normal'); // Fuente normal
  doc.text(`${venta.cliente.nombreCompleto}`, rightAlignMargin, 50, { align: 'right' });
  doc.text(`${venta.cliente.combinedIdentity}`, rightAlignMargin, 57.5, { align: 'right' });
  doc.text(`${formatDateTime(new Date(venta.fecha_registro))}`, rightAlignMargin, 65, { align: 'right' });
