...
const CustomMenuAdministracion = ({ itemName, itemOpen, BtnMostrar, BtnRegistrar1, nombreBtn1, BtnRegistrar2, nombreBtn2, BtnListar2, drawerOpen, icon }) => {
    ...
    <>
      ...
      <Collapse in={itemOpen} timeout="auto" unmountOnExit sx={{background: '#e2e2e2', minHeight: '100vh', display: 'flex'}}>
        ...
        <ListItem button onClick={BtnListar2} sx={{ display: 'flex', justifyContent: drawerOpen ? 'space-between' : 'center', border: '1px solid #0f1b35', color: '#0f1b35', transition: 'width 0.3s' }}>
          ...
          {drawerOpen && <ListItemText primary={`Lista de ${nombreBtn2}`} sx={{ marginLeft: 1, color: '#0f1b35', fontWeight: 'bold' }}/>}
        </ListItem>
...
export default CustomMenuAdministracion;
